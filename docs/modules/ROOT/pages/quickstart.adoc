...
Now start the Cerbos PDP server. We are using the container image in this guide but you can follow along using the binary as well. See xref:installation/binary.adoc[installation instructions] for more information.
...
NOTE: Each Cerbos policy must be in its own file. You cannot define multiple policies in a single YAML or JSON file.
