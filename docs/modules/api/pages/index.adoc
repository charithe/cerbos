...
See xref:recipes:query-plan-adapters/index.adoc[Query plan adapters] for installation, usage, and operator support details.


[#server-info]
=== `ServerInfo` (`/api/server_info`)

Returns Cerbos PDP server version.
...
Cerbos PDP partially implements the link:https://openid.github.io/authzen/[OpenID AuthZEN Authorization API specification], providing a standardized way to make authorization decisions. The AuthZEN API uses a simplified entity model that maps to Cerbos PDP's native authorization model.

[#authzen-entity-model]
...
AuthZEN entities are mapped to Cerbos PDP CheckResources requests as follows:
...
Returns metadata about the Cerbos PDP, including endpoint URLs for accessing the AuthZEN APIs.
...
<13> Auxiliary data such as JWT tokens. See xref:#check-resources[] for details
...
<3> Full Cerbos CheckResources response (included when `cerbos.includeMeta` is `true`). See xref:#check-resources[] for response structure details


[#authzen-access-evaluation-batch]
...
`execute_all` (default):: Process all evaluations regardless of individual results
`deny_on_first_deny`:: Stop processing after the first denial
`permit_on_first_permit`:: Stop processing after the first permit
...
<3> Full Cerbos PDP CheckResources response for this evaluation (included when `cerbos.includeMeta` is `true`)
